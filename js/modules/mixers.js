...
class MixerBase extends Module {
  constructor(channels, def) {
    super({ title: channels + " CH MIXER", width: 60 + channels * 46, ...def });
    ...
    for (let i = 0; i < channels; i++) {
      ...
      this.addPort(pr, "in", g, { label: "IN " + (i + 1) });
    ...
    m.className = "col"; m.style.alignItems = "center"; m.style.gap = "6px";
    m.style.borderLeft = "1px solid rgba(255,255,255,.08)"; m.style.paddingLeft = "8px";
    const mt = document.createElement("div"); mt.className = "tiny-label"; mt.textContent = "MASTER"; m.appendChild(mt);
    ...
    this.addPort(pr, "out", this.master, { label: "ST OUT" });
    strip.appendChild(m);
  }
}
